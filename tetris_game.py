...
class TetrisGame:
    def __init__(self):
        ...
        pygame.time.set_timer(self.game_update, 350)
        self.status = True
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    self.toggle_pause()
                elif not self.paused and self.controls_enabled :
    ...
    def draw(self):
        ...
        if self.paused:
            self.screen.blit(self.pause_text1, (350, 450, 50, 50))
            self.screen.blit(self.pause_text2, (350, 500, 100, 30))
